...
def go(bot, updater,user_data):
    if user_data['current']<len(user_data['english_words']):
        static_api_request = "https://www.googleapis.com/customsearch/v1?q="+user_data['english_words'][user_data['current']]+"&key= AIzaSyDFDVZ-c-Yy9iC50xOkNkvWoguzUD2-9q0&searchType=image&cx=005680010173931685076:cdr_fpoqyvm"
...
def start(bot, update,user_data):
    ...
    user_data['russian_words']="кот друзья платье юбка кофта кружка абрикос яблоко груша вишня торт пакет зеркало мебель глаза мяч брови уши цирк свадьба орел ножницы закат солнце футбол стопа кисть ракетка круг птица гора небо зима".split(" ")
    user_data['english_words']=[]
    for i in user_data['russian_words']:
        try:
            translator_uri = "https://translate.yandex.net/api/v1.5/tr.json/translate"
    ...
    update.message.reply_text(
        "Добрый день. Я бот, который помогает выучить английские слова. Чтобы начать обучение словам, уже заложенными в боте, пришлите команду  /go. Чтобы начать обучение Вашими словами, пришлите команду /slova с необходимыми словами(например: /slova груша солнце снег). Пришлите команду /stop, чтобы закончить обучение. (Переведено сервисом «Яндекс.Переводчик» http://translate.yandex.ru/.)")
    go(bot, updater,user_data)
...
#запасной ключ  AIzaSyBLxDLnQQJbB6Ps0oEcqUK5w-0Tg3rhLVI 